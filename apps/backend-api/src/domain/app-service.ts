...
export const appService = {
  ...
  async createCustomerContact(token: string, request: CustomerContactCreateRequest): Promise<Record<string, unknown>> {
    const created = await supabase.insert<Record<string, unknown>[]>('customer_contacts', token, {
      customer_id: request.customerId,
      name: request.name,
      role: request.role,
      email: request.email,
  ...
  async listAuditEvents(token: string, limit = 100): Promise<Record<string, unknown>[]> {
    const session = await this.sessionFromToken(token);
    const tenantId = encodeURIComponent(String(session.user.tenant_id));
    return supabase.query<Record<string, unknown>[]>(`audit_events?tenant_id=eq.${tenantId}&order=created_at.desc&limit=${Math.max(1, Math.min(limit, 500))}&select=*`, token);
  },
