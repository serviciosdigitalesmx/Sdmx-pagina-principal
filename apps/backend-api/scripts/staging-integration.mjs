...
const run = async () => {
  ...
  // RLS check: anonymous access must not expose tenant rows.
  ...
  if (anonymousRead.status === 200) {
    const anonPayload = await anonymousRead.json().catch(() => null);
    if (!Array.isArray(anonPayload) || anonPayload.length > 0) {
      throw new Error(`Expected anonymous read to return empty dataset, received: ${JSON.stringify(anonPayload)}`);
    }
  } else if (![401, 403].includes(anonymousRead.status)) {
    throw new Error(`Expected anonymous RLS guard (200-empty/401/403), received status ${anonymousRead.status}`);
  }
