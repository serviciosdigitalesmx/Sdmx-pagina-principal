    ...
    <RequireRole allowed={["owner", "manager", "technician"]}>
      <div className="space-y-6 text-slate-950">
        <header className="flex flex-col justify-between gap-4 rounded-[28px] border border-zinc-800 bg-zinc-950/85 p-6 shadow-[0_16px_70px_rgba(0,0,0,0.24)] sm:flex-row sm:items-center">
          ...
          <button onClick={() => setIsModalOpen(true)} className="rounded-full bg-slate-400 px-4 py-2.5 text-sm font-semibold text-zinc-950 transition hover:bg-slate-300">
            Nueva Orden
          </button>
        </header>
            ...
            return (
              <div key={column.id} className="min-w-[260px] rounded-[24px] border border-zinc-800 bg-zinc-950/85 shadow-[0_12px_50px_rgba(0,0,0,0.24)]">
                <div className="flex items-center justify-between border-b border-slate-200 px-4 py-3">
                  <h2 className="text-sm font-semibold uppercase tracking-[0.22em] text-[#1f2937]">{column.title}</h2>
                ...
                <div className="space-y-3 p-3">
                  {columnOrders.map((order) => (
                    <article key={order.id} className="cursor-pointer rounded-2xl border border-zinc-800 bg-zinc-900/60 p-4 shadow-sm transition hover:-translate-y-0.5 hover:border-slate-400/30" onClick={() => order.id && void openOrder(order.id)}>
                      <div className="flex items-start justify-between gap-3">
                        <div>
                          <p className="text-xs font-semibold uppercase tracking-[0.22em] text-[#1f2937]">{order.folio ?? "ORD-..."}</p>
                          <p className="mt-1 text-sm font-semibold text-slate-950">{order.device_info?.customer_name ?? "Cliente sin nombre"}</p>
                        </div>
                        <span className="rounded-full bg-[#475569]/10 px-2.5 py-1 text-[11px] font-semibold uppercase tracking-[0.18em] text-[#475569]">Activo</span>
                      </div>
                      ...
                      <div className="mt-3 flex flex-wrap gap-2">
                        {whatsappLink(getDetailPhone(order), tenantSlug, customerPortalBase, order.folio) ? (
                          <a
                            ...
                            target="_blank"
                            rel="noreferrer"
                            className="rounded-full bg-[#475569] px-3 py-1 text-xs font-semibold text-white"
